review = LOAD 'YelpInput/1000rows.csv' USING PigStorage(',') AS (business_id:chararray,date:chararray,review_id:chararray,stars:double,text:chararray,type:chararray,user_id:chararray,votes_cool:int,votes_funny:int,votes_useful:int);
positive = LOAD 'YelpInput/positive-words.txt' USING PigStorage('\n') AS (positiveWords:chararray);
negative = LOAD 'YelpInput/negative-words.txt' USING PigStorage('\n') AS (negativeWords:chararray);
...
result = FOREACH finalDiff GENERATE group,COUNT(diffe);
dump result;